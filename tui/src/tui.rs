...
fn ui(frame: &mut Frame, data_list: &DataList) {
    ...
    let summary_arr = [
        ...
        Row::new(["Price:".to_string(), format!("{:.2}", data_list.data[curr_i].last_trade_price)]),
        Row::new(["Quantity:".to_string(), data_list.data[curr_i].last_trade_qty.to_string()]),
        Row::new([
            "Last Trade:".to_string(), 
            DateTime::from_timestamp_millis(data_list.data[curr_i].last_trade_time)
                .unwrap().with_timezone(&Local).to_string()
        ]),
    ];
    frame.render_widget(
        Table::new(summary_arr, [Constraint::Length(12), Constraint::Length(30)])
        ...
        for _ in 0..(self.capacity - 1) {
            ...
            data_vec.push(
                Row::new([
                    ...
                    Cell::new(DateTime::from_timestamp_millis(self.data[i].timestamp_ms)
                        .unwrap().with_timezone(&Local).format("%H:%M:%S").to_string()),
                    Cell::new(format!("{:.2}", self.data[i].best_bid))
                        .fg(bid_fg_color),
                    Cell::new(format!("{:.2}", self.data[i].best_ask))
                        .fg(ask_fg_color),
                ])
